    ...
    `);
        ...
        '2025-06-13 08:30:00', 45, 'Adelaide University', 'completed')`);
            ...
            (9, (SELECT user_id FROM Users WHERE username = 'christinewalker'),
                ...
                4, 'Good Service', '2025-06-12 18:00:00')
                `);
        }
    } catch (err) {
    ...
    try {
        const [rows] = await db.execute(`
            SELECT
                ...
                CASE
                    WHEN CONUT(wr.rating_id) > 0 THEN ROUND(AVG(wr.rating), 1)
                    ELSE NULL
                END AS average_rating,
                COUNT(DISTINCT CASE
                    WHEN wa.status = 'accepted' AND req.status = 'completed'
                    THEN wa.request_id
                END) AS completed_walks
            FROM Users u
            LEFT JOIN WalkApplications wa ON u.user_id = wa.walker_id
            LEFT JOIN WalkRatings wr ON u.user_id = r.walker_id
            LEFT JOIN WalkRequests req ON wa.request_id = r.request_id AND wr.status = 'completed'
            WHERE u.role = 'walker'
            GROUP BY u.user_id
            `);
        res.json(rows);
